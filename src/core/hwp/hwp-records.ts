...
export function readHwpRecord(bytes: Uint8Array, offset: number): HwpRecord | null {
...
export function readHwpRecords(bytes: Uint8Array, limit = 500000): HwpRecord[] {
...
export function decodeParaText(body: Uint8Array): string {
  ...
  for (let offset = 0; offset + 2 <= body.length; offset += 2) {
    ...
    if (isInlineControlCode(code)) continue;
...
export function scanUtf16Text(bytes: Uint8Array, minimumBytes = 20): string {
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  let bestStart = -1;
  let bestLength = 0;
  let bestScore = 0;
...
export function scoreRecords(records: HwpRecord[]): number {
  let score = records.length;
  for (const record of records) {
    if (record.tagId === HWP_TAG.PARA_TEXT) score += decodeParaText(record.body).replace(/\s+/g, '').length * 20;
    if (record.tagId === HWP_TAG.PARA_HEADER) score += 5;
    if (record.tagId === HWP_TAG.PARA_CHAR_SHAPE || record.tagId === HWP_TAG.PARA_LINE_SEG) score += 2;
    if (record.tagId === HWP_TAG.CTRL_HEADER || record.tagId === HWP_TAG.LIST_HEADER) score += 4;
    if (record.tagId === HWP_TAG.TABLE || record.tagId === HWP_TAG.SHAPE_COMPONENT_PICTURE) score += 10;
  }
  return score;
}

function isExtendedControlCode(code: number): boolean {
  return code === 0x0002 || code === 0x0003 || code === 0x000b || (code >= 0x000f && code <= 0x0017);
}

function isInlineControlCode(code: number): boolean {
